...
export class HistorialComponent implements OnInit, OnDestroy {
    ...
    if (dataElemento.anulacion) {
      ...
      this.subscripciones.push(
        this.dialogService
          .open(MensajeEntradaComponent, {
            context: {
              titulo: mensaje, mensajeAdvertencia: "Si el requerimiento de este transporte solo posee este registro, el requerimiento pasará a la fase de 'Terminado'"
            },
          })
          .onClose.subscribe((res) => {
            if (res) {

              this.validacionRequerimieno(dataElemento, "'" + res + "', por " + this.auth.getUserStorage().email + " el " + new Date().toLocaleString());
            }
          })
      );
    } else {
      ...
      this.subscripciones.push(
        this.dialogService
          .open(MensajeEntradaComponent, {
            context: {
              titulo: mensaje, mensajeAdvertencia: "Si este es el único registro perteneciente al requerimiento se cambiará su estado a 'aprobado'"
            },
          })
          .onClose.subscribe((res) => {
            if (res) {
              this.anular(dataElemento, "'" + res + "', por " + this.auth.getUserStorage().email + " el " + new Date().toLocaleString());
            }
          })
      );
    }
  ...
  private validacionRequerimieno(transporte: any, motivoActivacion: string): void {
  ...
  private cambiarEstadoRequerimiento(transporte: any, motivoActivacion: string): void {
    ...
    this.subscripciones.push(
      this.requerimientosService.editar(requerimiento.idRequerimiento, requerimiento)
        .subscribe(
          (res) => {
            Util.showToast("success", "Acción realizada", "Se ha cambiado el estado al requerimiento", 4000, this.toastrService);
            this.reactivarTransporte(transporte, motivoActivacion);
          },
          (error) => {
            console.error(error);
            Util.showToast("danger", "Error " + error.status, "Mientras se cambiaba el estado al requerimiento " + error.error[0], 0, this.toastrService);
          }
        )
    );
  }
  ...
  private reactivarTransporte(transporte: any, motivo: string): void {
    this.subscripciones.push(
      this.transporteService.anular(transporte.idTransporte, motivo)
        .subscribe(
          (res) => {
          ...
          (error) => {
            console.error(error);
            Util.showToast("danger", "Error " + error.status, "Mientras se reactivaba el registro " + error.error[0], 0, this.toastrService);
          }
        )
    );
  }
  ...
  private evaluacionRequerimientos(elemento: any): void {
    ...
    this.subscripciones.push(
      this.transporteService.listarActivosPorRequerimiento(elemento.requerimientoId.idRequerimiento).subscribe(
        (res) => {
        ...
        (error) => {
          console.error(error);
          Util.showToast("danger", "Error " + error.status, "Mientras se verificaban los registros pertenecientes al requerimiento " + error.error[0], 0, this.toastrService);
        }
      ));
  ...
  private cambioEstadoRequerimiento(elemento: any): void {
    ...
    this.subscripciones.push(
      this.requerimientosService.editar(elemento.idRequerimiento, elemento).subscribe(
        () => {
          Util.showToast("success", "Acción realizada", "Se ha cambiado el estado al requerimiento perteneciente ", 4000, this.toastrService);
        },
        (error) => {
          console.error(error);
          Util.showToast("danger", "Error " + error.status, "Mientras se cambiaba el estado al requerimiento " + error.error[0], 0, this.toastrService);
    ...
    this.subscripciones.push(
      this.transporteService
        ...
        .subscribe(
          ...
          (error) => {
            console.error(error),
              Util.showToast("danger", "Error " + error.status, "Mientras se realizaba la búsqueda de los transportes " + error.error[0], 0, this.toastrService);
          }
        )
    );
  }
