...
export class ListadoComponent implements OnInit, OnDestroy {
  ...
  data: any;
  constructor(private categoriaService: CategoriaService, private dialogService: NbDialogService, private toastrService: NbToastrService) { }

  construir(): void {
    this.subscripciones.push(this.categoriaService.listar().subscribe((resp: any) => {
      this.data = resp;
      this.dtTrigger.next();
    }, error => {
      console.error(error);
  ...
  eliminar(id): void {
    this.subscripciones.push(this.categoriaService.eliminar(id.idCategoria).subscribe(res => {
      if (res) {
        this.showToast('success', 'Acción realizada', 'Se ha eliminado el registro', 4000);

      } else {
        this.showToast('warning', 'Atención', 'No se ha encontrado el registro', 4000);
      }
      this.reconstruir(id);
    }, error => {
      console.error(error);
