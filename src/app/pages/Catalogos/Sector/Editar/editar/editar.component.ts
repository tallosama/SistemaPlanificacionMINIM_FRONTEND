...
export class EditarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscripciones.push(this.sectorService.buscar(this.id).subscribe(res => {
      this.editarSectorForm = this.fb.group(
        {
          desSector: [res.desSector, Validators.compose([Validators.required, Validators.maxLength(512)])],
  ...
  public editar(): void {
    this.subscripciones.push(this.sectorService.editar(this.id, this.editarSectorForm.value).subscribe(resp => {

      this.router.navigate(['../../ListarSector'], { relativeTo: this.route });
